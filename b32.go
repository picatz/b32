...
func encode(input string) string {
	return string(base32.StdEncoding.EncodeToString([]byte(input)))
}

func decode(input string) string {
	decodedString, _ := base32.StdEncoding.DecodeString(input)
...
func main() {
	...
	app.Name = "b32"
	app.Version = "1.0.0"
	app.Usage = "base32 encoder and decoder"

	app.Commands = []cli.Command{
		{
			Name:  "encode",
			Usage: "encode the given string(s) as base32",
		...
		{
			Name:  "decode",
			Usage: "decode the given string(s) as base32",
			Action: func(c *cli.Context) error {
				if pipeGiven() {
					for input := range readStdin() {
